...
def evaluate_ckf(params: dict) -> dict:
    ...
    try:
        ...
        # ---- pull metrics ----------------------------------------
        with uproot.open(workdir / "performance_ckf.root") as rh:
            eff   = rh["eff_particles"].member("fElements")[0]
            fake  = rh["fakerate_tracks"].member("fElements")[0]
            dup   = rh["duplicaterate_tracks"].member("fElements")[0]

        tdf = pd.read_csv(workdir / "timing.tsv", sep="\t")

        run_match = tdf.loc[
            tdf["identifier"].str.contains("Algorithm:TrackFindingAlgorithm"),
            "time_perevent_s",
        ]

        run_time = float(run_match.iloc[0]) if not run_match.empty else float("nan")
...
def main(argv=None):
    ...
    # ─── build evaluator, generator, Xopt object ───────────────────
    ...
    if MPI.COMM_WORLD.Get_rank() == 0:
        # 1️⃣ initial random points + optimisation loop
        X.random_evaluate(5)
